...
@Component({
  ...
  template: `
    <div
      ...
      <span>
        <span class="text-slate-500 select-none">$ </span>npm install <span
          class="text-blue-400 font-semibold">{{ package() }}</span>
      </span>
      <button (click)="copy()"
        ...
        @if (copied()) {
        <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 text-green-400" fill="none" viewBox="0 0 24 24"
